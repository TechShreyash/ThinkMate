...
- **Engagement commands** — the Phase 12 DM commands (`/help`, `/onboard`, `/checkins`, `/reactions`) and the `/start` inline guide/menu.
...
`/start`, `/help`, `/onboard`, `/checkins`, `/profile`, `/reset`, and `/reactions` are implemented. `/reset` requires explicit
...
Phase 12 adds small, DM-oriented commands plus light enhancements to `/start` and its inline guide.
...
### `/help` — direct command discovery

`/help` opens the same grouped command cheat sheet used by the inline *Commands* guide screen.
It exists for new users who instinctively try a help command before exploring buttons. The text is
generated by `_build_help_text(is_admin)`, so it honors renamed/disabled commands and hides admin
commands from non-admins.
...
### `/start` — welcome and guide entry point
...
command list (*Commands*). Users can also send `/help` to jump straight to the command cheat sheet.
...
single message. The full command list is also available directly via `/help`.
...
`_build_help_text(is_admin)` builder, the same builder used by `/help` (admin-only commands are
...
Telegram's native **"/" command menu** publishes a short self-service menu: at startup `main.py` calls
[`setup_bot_commands(bot)`](../../app/handlers/commands.py) (once the router is wired but before
polling begins), which publishes DM commands in the default scope (`BotCommandScopeDefault`) and
group-safe controls in the group scope (`BotCommandScopeAllGroupChats`).

The DM menu currently includes `/start`, `/help`, `/onboard`, `/checkins`, `/profile`, `/reset`,
and `/reactions`. The group menu includes `/start`, `/help`, `/quiet`, `/chatty`, `/groupbot`, and
`/groupmode`. Admin-only `/health` and `/metrics` are deliberately omitted from public command menus.
...
Both menus are built by `_menu_for(...)`, which
honors each command's **resolved trigger and enabled flag** from `config.COMMANDS` (a renamed
