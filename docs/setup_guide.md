...
| `CHAT_BUFFER_MAX_CHARS` | Integer | `4000` | Max character count allowed in the buffer before extraction triggers. |
...
| `NEW_USER_MEMORY_THRESHOLD` | Integer | `10` | A user with fewer than this many stored memory items (facts + beliefs + events) is treated as "new/sparse" and uses `NEW_USER_EXTRACTION_CHARS`. |
| `CHAT_BUFFER_TRIM` | Integer | `8` | The count of latest messages preserved in active history when a buffer trim is executed. |
...
| `MESSAGE_BATCH_DELAY_SECS` | Float | `2.0` | Delay in seconds the bot waits after receiving a message to batch rapid messages. |
| `MAX_BATCH_DELAY_SECS` | Float | `6.0` | Max seconds from first message in a batch before processing is forced. |
| `MAX_INPUT_CHARS` | Integer | `4000` | Inbound messages longer than this are ignored (anti-abuse — blocks pasted logs/essays), not a normal chat cap. |
| `MAX_RESPONSE_CHARS` | Integer | `4000` | App-level cap for generated replies before sending. Telegram delivery still splits text into safe chunks below the 4096-character message limit. Set `0` to disable the app cap while keeping Telegram chunking. |
