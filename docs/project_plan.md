...
-   [x] **1.2 Dependencies Setup**: Create the [requirements.txt](../requirements.txt) file listing `aiogram`, `python-dotenv`, `aiosqlite`, `openai`, `pydantic`, and `loguru`.
-   [x] **1.3 Environment Variables Configuration**: Author the [.env.example](../.env.example) configuration template with Telegram token slots and local/cloud LLM endpoint variables.
-   [x] **1.4 Typed Configuration System**: Write [config.py](../app/config.py) to load environment variables via `python-dotenv`, execute type parsing, and validate configurations.
...
-   [x] **1.6 Logging Subsystem**: Initialise `loguru` in [app/\_\_init\_\_.py](../app/__init__.py) to log outputs to standard output and rotating log files.
...
-   [x] **2.1 Connection Singleton**: Write [connection.py](../app/database/connection.py) to handle database initialization and configure SQLite's WAL (Write-Ahead Logging) mode.
-   [x] **2.2 Schema Implementation**: Define SQL schemas in [connection.py](../app/database/connection.py) or an independent `.sql` file for the following tables:
...
-   [x] **2.3 Dependency-Injected CRUD Models**: Write python database accessors in [models.py](../app/database/models.py) accepting `db: Connection` as their first parameter. Ensure transactional methods like `save_extracted_memories` take `MemoryExtraction` Pydantic models directly as inputs.
...
-   [x] **3.1 LLM Service Class**: Implement the core `LLMService` in [llm_service.py](../app/services/llm_service.py) wrapping `openai.AsyncOpenAI`.
...
-   [x] **3.3 Base Prompts Definition**: Write basic prompts under `app/prompts/`:
    *   [system_prompt.py](../app/prompts/system_prompt.py): Combines core instructions, bot persona, and active memories.
    *   [extraction_prompt.py](../app/prompts/extraction_prompt.py): Standardizes rules for extracting JSON key-value updates from text.
    *   [consolidation_prompt.py](../app/prompts/consolidation_prompt.py): Explains how to merge, deduplicate, and clean old memories.
...
-   [x] **4.1 Memory Loader**: Implement [memory_loader.py](../app/services/memory_loader.py) to load details, active facts, recent events, and current moods from SQLite, formatting them into a structured text context.
-   [x] **4.2 Memory Extractor**: Implement [memory_extractor.py](../app/services/memory_extractor.py) to select the oldest $N$ messages from the chat buffer, feed them to the extraction prompt, write updates to database tables, and trim the buffer.
-   [x] **4.3 Chat Manager Orchestrator**: Write [chat_manager.py](../app/services/chat_manager.py) to coordinate:
...
-   [x] **5.1 Entrypoint Script**: Initialise the bot, async dispatcher, and database connection loop in [main.py](../main.py).
...
-   [ ] **5.4 Command Handlers**: Implement slash commands in [commands.py](../app/handlers/commands.py):
...
-   [x] **5.5 Text Routing Handler**: Write message interception in [messages.py](../app/handlers/messages.py) marked with `flags={"long_operation": True}`.
...
-   [x] **6.1 Memory Compression Service**: Create [memory_compressor.py](../app/services/memory_compressor.py) to run LLM-powered memory consolidation in background when budget is exceeded.
...
-   [x] **6.4 Throttling and Queue Guards**: Implement `ThrottlingMiddleware` for early memory-level rate limiting and `MAX_QUEUED_MESSAGES` to prevent database/memory queue bloat.
...
-   [x] **7.1 Unit Testing Framework**: Set up `pytest` configuration and test fixtures.
-   [x] **7.2 Database Test Cases**: Create [test_database.py](../tests/test_database.py) to test SQLite transactions, WAL configurations, and per-user isolation.
-   [x] **7.3 Memory Engine Test Cases**: Write mock LLM test suites in [test_memory_extractor.py](../tests/test_memory_extractor.py) and [test_batching_and_concurrency.py](../tests/test_batching_and_concurrency.py) verifying memory formatting, batching, queuing, and serialization.
