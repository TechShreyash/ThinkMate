...
You will receive the user's COMPLETE current memory profile containing 5 components:
...
3. **Subjective Beliefs** — The user's personal convictions, values, and opinions.
4. **Events** — Chronological life events with dates and significance levels.
5. **Current Emotional State** — Latest emotional state with mood tag, intensity, and trigger.
...
1. **ALWAYS KEEP**: Identity facts (name, age, location, family), core subjective beliefs/values, major life events (marriage, job change, graduation), active health conditions, strong preferences that define the user.
2. **MERGE when possible**: Similar or overlapping facts/beliefs/events → combine into one concise statement. Example: "Enjoys green tea" + "Prefers organic green tea" → "Prefers organic green tea".
3. **KEEP BUT SHORTEN**: Minor life events — reduce verbose descriptions to brief summaries. Keep the date and significance.
4. **DROP**: Routine/trivial events ("bought groceries", "had lunch"), outdated temporary states ("feeling tired today" from weeks ago), duplicate or superseded facts/beliefs.
...
- **compressed_beliefs**: Each belief must be a single concise sentence. Merge overlapping or related beliefs where possible.
...
- **emotional_state**: Always preserve the latest emotional state — this is the user's current mood and is critical for conversation tone.

### Data Integrity:
- Do NOT invent, assume, or hallucinate facts/beliefs/events that are not in the input.
- Do NOT change the meaning of any fact, belief, or event — only condense wording.
- Do NOT merge items from different categories.
