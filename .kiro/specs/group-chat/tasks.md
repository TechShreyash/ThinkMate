...
- [ ] 1. Data model & schema foundation
  ...
  - [ ] 1.1 Extend reply/extraction schemas for groups
  ...
  - [ ] 1.2 Generalize the buffer to `chat_id` + sender attribution
  ...
  - [ ] 1.4 Tests: buffer attribution + chat_members CRUD
...
- [ ] 2. Chat-context plumbing through orchestrator & task manager
  ...
  - [ ] 2.1 Add chat context to `handle_message` + multi-party rendering
...
- [ ] 3. Group routing & bot identity
  ...
  - [ ] 3.1 Implement `group_gate` identity + cheap-scan helpers
...
- [ ] 4. Ambient gate (no-LLM funnel)
  ...
  - [ ] 4.1 Implement `AmbientGate` (cooldown → scan tick → affinity dice → prune)
...
- [ ] 5. Affinity store, signals & commands
  ...
  - [ ] 5.1 Implement `AffinityCache` (read-through / write-through)
